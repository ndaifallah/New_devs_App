...
class Settings(BaseSettings):
    ...
    supabase_url: Optional[str] = "postgres"
    supabase_db_user: Optional[str] = "postgres"
    supabase_db_password: Optional[str] = "postgres"
    supabase_db_host: Optional[str] = "postgres"
    supabase_db_port: Optional[str] = "5432"
    supabase_db_name: Optional[str] = "propertyflow"
    ...
    def __init__(self, **kwargs):
        ...
        for field_name, value in loaded_values.items():
            if value:
                ...
                logger.info(
                    f"✅ LOADED {field_name}: {preview} (len: {len(str(value))})"
                )
            else:
    ...
    database_retry_delay: float = (
        0.5  # Base delay between retries (exponential backoff)
    )

    # Supabase Connection Management
    supabase_max_concurrent_connections: int = (
        150  # Max concurrent Supabase connections (increased for performance)
    )
